...
celesteBajo = np.array([50, 200, 0], np.uint8)
celesteAlto = np.array([80, 255, 255], np.uint8)
...
while True:
	...
	frameconpunto=copy.deepcopy(frame)
	if results.multi_hand_landmarks:
		...
		cv2.circle(frameconpunto, pixelCoordinatesLandmark, 2, (0, 200, 0), 15)
	frameHSV = cv2.cvtColor(frameconpunto, cv2.COLOR_BGR2HSV)
	...
	cv2.putText(frame, 'Limpiar', (320, 20), 6, 0.6,
				colorLimpiarPantalla, 1, cv2.LINE_AA)
	cv2.putText(frame, 'pantalla', (320, 40), 6, 0.6,
				colorLimpiarPantalla, 1, cv2.LINE_AA)
	...
	for c in cnts:
		...
		if pausardibujo==False:# reemplazo area > 1000: por true para que lo haga siempre y no cuando esta cerca
			...
			if x1 is not None:
				...
				if 300 < x2 < 400 and 0 < y2 < 50:
					...
					cv2.putText(frame, 'Limpiar', (320, 20), 6, 0.6,
								colorLimpiarPantalla, 2, cv2.LINE_AA)
					cv2.putText(frame, 'pantalla', (320, 40), 6, 0.6,
								colorLimpiarPantalla, 2, cv2.LINE_AA)
	...
	cv2.imshow('Pizarra',imAux)
	cv2.imshow('Camara Web', frame)
	k = cv2.waitKey(1)
	if k == 27:
		break
	if k == 32: #Barra espaciadora
		pausardibujo=not(pausardibujo)
	if k == 115 or k == 83:		#S MAYUSCULA o minuscula
		fecha=time.strftime("%Y%m%d-%H%M%S")
		nombre=f"dibujo_{fecha}.png"
		cv2.imwrite(str(nombre), imAux)
